      ...
      <table className="w-full text-sm text-left text-taupe">
        ...
        <tbody>
          {evaluationMetrics.ethics.map((metric, index) => (
            <tr key={index} className="bg-white border-b border-tan/50">
              <th scope="row" className="px-6 py-4 font-medium text-navy whitespace-nowrap">{metric.name}</th>
              {models.map(model => {
                const modelKey = getModelKey(model.name);
                const result = getEthicsScore(modelKey, metric.criterion);
                return (
                  <td key={model.name} className="px-6 py-4 text-center">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${result.completed ? 'bg-slate-blue/10 text-slate-blue' : 'bg-tan/50 text-taupe'}`}>
                    ...
                    <td key={model.name} className="px-6 py-4 text-center">
                      {result ? (
                        <div className="flex flex-col items-center">
                          <span className="font-semibold text-slate-blue">{result.percentage}%</span>
                          <span className="text-xs text-taupe">({result.grade} 등급)</span>
                        </div>
                      ) : (
                        <span className="text-taupe">미평가</span>
                      )}
                    </td>
                  )
                })}
      ...
      <table className="w-full text-sm text-left text-taupe">
        ...
        <tbody>
          {evaluationMetrics.scenarioDeepEval.map((metric, index) => (
            <tr key={index} className="bg-white border-b border-tan/50">
              <th scope="row" className="px-6 py-4 font-medium text-navy whitespace-nowrap">{metric.name}</th>
              <td className="px-6 py-4"><span className="text-xs bg-tan/50 text-taupe px-2 py-1 rounded-md">{metric.category}</span></td>
              {models.map(model => <td key={model.name} className="px-6 py-4 text-center text-taupe">평가 예정</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const renderExpertPanel = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {evaluationMetrics.expert.map((item, index) => (
        <div key={index} className="bg-white p-6 rounded-xl shadow-md border border-tan/30">
          <h4 className="text-lg font-semibold text-navy mb-2">{item.name}</h4>
          <p className="text-sm text-taupe">{item.description}</p>
          <button className="mt-4 w-full text-sm font-medium text-slate-blue bg-cream py-2 rounded-lg hover:bg-tan">
            자세히 보기
          </button>
        </div>
      ))}
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-tan/50 sticky top-0 z-40">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4 py-4">
            ...
            <h1 className="text-xl font-bold text-navy">모델 비교 분석</h1>
          </div>
        </div>
      </header>
      <main className="py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div className="mb-8 text-center">
          <h2 className="text-3xl font-bold text-navy mb-2">AI 모델 상세 비교</h2>
        ...
        <div className="mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {models.map(model => (
              <div key={model.name} className="bg-white p-6 rounded-xl shadow-lg border border-tan/30">
                <div className="flex items-center">
                  <model.icon className="w-8 h-8 text-navy mr-4" />
                  <div>
                    <h3 className="text-lg font-semibold text-navy">{model.name}</h3>
                    <p className="text-sm text-taupe">{model.provider}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white p-2 rounded-xl shadow-lg border border-tan/30 mb-8">
          <div className="flex items-center justify-center space-x-1">
            {tabs.map(tab => (
                ...
                className={`w-full px-4 py-2.5 text-sm font-semibold rounded-lg transition-colors 
                  ${activeTab === tab.id ? 'bg-navy text-white shadow' : 'text-taupe hover:bg-cream'}`}
              >
                  {tab.name}
                </button>
              ))}
