...
export default function EvaluationDataPage() {
  ...
  const InputField = ({ id, value, onChange, placeholder, required = false }) => (
    <input
      id={id}
      type="text"
      value={value}
      onChange={onChange}
      className="w-full px-4 py-2 bg-cream border border-tan/50 rounded-lg focus:ring-slate-blue focus:border-slate-blue text-navy placeholder-taupe"
      placeholder={placeholder}
      required={required}
    />
  );

  const TextareaField = ({ id, value, onChange, placeholder }) => (
    <textarea
      id={id}
      value={value}
      onChange={onChange}
      className="w-full px-4 py-2 bg-cream border border-tan/50 rounded-lg focus:ring-slate-blue focus:border-slate-blue text-navy placeholder-taupe h-24 resize-none"
      placeholder={placeholder}
    />
  );
    ...
    <div className="bg-cream min-h-screen">
      <header className="bg-white/80 backdrop-blur-sm border-b border-tan/50 sticky top-0 z-40">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4 py-4">
            ...
            <h1 className="text-xl font-bold text-navy">커스텀 설정</h1>
          </div>
        </div>
      </header>
      ...
      <main className="py-8 mx-auto max-w-5xl sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-navy mb-2">모델 및 평가지표 추가</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-6 rounded-xl shadow-lg border border-tan/30">
            <h3 className="text-lg font-semibold text-navy mb-4 flex items-center">
              <PlusCircleIcon className="w-6 h-6 mr-2 text-slate-blue" />
              새 모델 추가
            </h3>
            <form onSubmit={handleAddModel} className="space-y-4">
              <div>
                <label htmlFor="modelName" className="block text-sm font-medium text-taupe mb-1">모델 이름</label>
                <InputField id="modelName" value={modelName} onChange={e => setModelName(e.target.value)} placeholder="예: GPT-4-turbo" required />
              </div>
              <div>
                <label htmlFor="modelDesc" className="block text-sm font-medium text-taupe mb-1">설명</label>
              ...
              <button type="submit" disabled={user?.isGuest} className="w-full bg-slate-blue text-white font-semibold py-2.5 rounded-lg hover:bg-navy transition-colors disabled:bg-taupe/50 disabled:cursor-not-allowed">
                모델 추가하기
              </button>
            </form>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-tan/30">
            <h3 className="text-lg font-semibold text-navy mb-4 flex items-center">
              <BeakerIcon className="w-6 h-6 mr-2 text-slate-blue" />
              새 평가지표 추가
            </h3>
            <form onSubmit={handleAddMetric} className="space-y-4">
              <div>
                <label htmlFor="metricName" className="block text-sm font-medium text-taupe mb-1">지표 이름</label>
                <InputField id="metricName" value={metricName} onChange={e => setMetricName(e.target.value)} placeholder="예: 정답률 (Accuracy)" required />
              </div>
              <div>
                <label htmlFor="metricDesc" className="block text-sm font-medium text-taupe mb-1">설명</label>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-6 rounded-xl shadow-lg border border-tan/30">
            <h3 className="text-lg font-semibold text-navy mb-4">등록된 모델 목록</h3>
            {models.length === 0 ? (
              <p className="text-sm text-taupe text-center py-4">등록된 모델이 없습니다.</p>
            ) : (
              <ul className="space-y-2">
                {models.map((m) => (
                  <li key={m.id} className="p-3 bg-cream rounded-lg text-sm">
                    <strong className="text-navy">{m.name}</strong>
                    {m.description && <span className="text-taupe ml-2">- {m.description}</span>}
                  </li>
                ))}
              </ul>
            )}
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-tan/30">
            <h3 className="text-lg font-semibold text-navy mb-4">등록된 평가지표 목록</h3>
            {metrics.length === 0 ? (
              <p className="text-sm text-taupe text-center py-4">등록된 지표가 없습니다.</p>
            ) : (
              <ul className="space-y-2">
                {metrics.map((met) => (
                  <li key={met.id} className="p-3 bg-cream rounded-lg text-sm">
                    <strong className="text-navy">{met.name}</strong>
                    {met.description && <span className="text-taupe ml-2">- {met.description}</span>}
                  </li>
                ))}
              </ul>
            )}
